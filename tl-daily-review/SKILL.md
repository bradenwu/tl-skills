...
description: |
  ...
  除活动统计外，还支持复盘后的可选增值动作：基于事实基线的启发式发散、以及针对近期摄入内容的知识检索回顾（retrieval quiz）。
...
以下动作**均非默认**，需用户在复盘后明确发起（如"出几道检索题""帮我发散一下接下来能做什么"）。默认流程到「追加到当天日志」即止。
...
3. 术语/定义回忆用**填空**题
4. 每题带 `why`（解析）+ `ref`（出处）；答错时尤其要把出处讲清

**产物**：用 `scripts/quiz_template.html` 生成单文件 HTML，写到当天日志旁（如 `04_日志/YYYY-MM-DD-review.html`）。流程：扫描阅读笔记 → 读模板 → 填题库 `Q` 数组 → 写出文件，无需从零写样式。
